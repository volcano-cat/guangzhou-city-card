...
export default function FoodPage() {
  ...
  const fetchCategories = async () => {
    try {
      const res = await axios.get("/api/categories?type=FOOD")
      if (res.data.success) {
  ...
  const fetchFoods = async (searchKeyword = keyword) => {
    setLoading(true)
    try {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900">广州美食</h1>
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          <p className="text-gray-500">暂无美食数据</p>
        </div>
      ) : (
        <>
