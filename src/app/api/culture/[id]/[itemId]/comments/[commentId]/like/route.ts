...
export async function POST(
  request: NextRequest,
  { params }: { params: { id: string, itemId: string, commentId: string } }
) {
  try {
    ...
    const comment = await prisma.cultureComment.findUnique({
      where: { id: commentId },
    })
    ...
    const existingLike = await prisma.cultureCommentLike.findUnique({
      where: {
        userId_commentId: {
          userId: user.userId,
          commentId,
    ...
    await prisma.cultureCommentLike.create({
      data: {
        userId: user.userId,
        commentId,
      },
...
export async function DELETE(
  ...
  try {
    ...
    await prisma.cultureCommentLike.deleteMany({
      where: {
        userId: user.userId,
        commentId,
      },
