...
export default function AttractionDetailPage() {
  ...
  useEffect(() => {
    if (attraction) {
      let sortedComments = [...attraction.comments]
      ...
      // 只有当评论顺序真正改变时才更新状态
      const commentsChanged = JSON.stringify(attraction.comments) !== JSON.stringify(sortedComments)
      if (commentsChanged) {
        // 更新状态，使用排序后的评论
        setAttraction({
          ...attraction,
  ...
  const fetchAttraction = async () => {
    setLoading(true)
    try {
      const res = await axios.get('/api/attractions/' + params.id)
      if (res.data.success) {
  ...
  const handleFavorite = async () => {
    if (!user) {
      router.push('/login?redirect=/attractions/' + params.id)
      return
    }

    try {
      if (isFavorited) {
        await axios.delete('/api/attractions/' + params.id + '/favorite', { withCredentials: true })
        setIsFavorited(false)
      } else {
        await axios.post('/api/attractions/' + params.id + '/favorite', {}, { withCredentials: true })
        setIsFavorited(true)
      }
    } catch (error) {
  ...
  const handleSubmitComment = async () => {
    ...
    try {
      const res = await axios.post(
        '/api/attractions/' + params.id + '/comments',
        { content: commentContent, rating: commentRating },
        { withCredentials: true }
      )
      if (res.data.success) {
        setCommentContent('')
        // 直接更新评论列表，而不是重新获取整个景点信息
        if (attraction) {
          // 构建新评论对象
          const newComment = {
            id: res.data.data.id,
          ...
          // 添加到评论列表
          const updatedComments = [newComment, ...attraction.comments]
          // 重新排序
          ...
          setAttraction({
            ...
            _count: {
              ...attraction._count,
              comments: attraction._count.comments + 1
            }
          })
        }
      }
    ...
    try {
      ...
      if (res.data.success) {
        ...
        if (attraction) {
          ...
          // 找到父评论并添加回复
          const updatedComments = attraction.comments.map(comment => {
  ...
  const handleLike = async (commentId: number) => {
    ...
    try {
      const res = await axios.post(
        '/api/attractions/' + params.id + '/comments/' + commentId + '/like',
        {},
        { withCredentials: true }
      )
      if (res.data.success) {
  ...
  if (!attraction) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-16 text-center">
        <h1 className="text-2xl font-bold text-gray-900">景点不存在</h1>
      </div>
    )
  }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2">
            ...
            <div className="p-6">
              ...
              <div className="flex items-center space-x-4 text-sm text-gray-500 mb-6">
                <span className="flex items-center">
                  ...
                  {attraction.rating ? Number(attraction.rating).toFixed(1) : '暂无评分'}
                </span>
                <span>{attraction.viewCount} 次浏览</span>
                <span>{attraction._count.favorites} 人收藏</span>
                <span>{attraction._count.comments} 条评论</span>
              </div>

              <div className="prose max-w-none">
                <h2 className="text-xl font-semibold mb-3">景点介绍</h2>
          ...
          <div className="mt-8 bg-white rounded-lg shadow-md p-6">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-semibold">用户评论 ({attraction._count.comments})</h2>
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-lg shadow-md p-6 sticky top-24">
            <h3 className="text-lg font-semibold mb-4">景点信息</h3>
            
            <div className="space-y-4">
              <div>
                <div className="text-sm text-gray-500 mb-1">地址</div>
                <a 
                  href={`/map?type=attraction&id=${attraction.id}&name=${encodeURIComponent(attraction.name)}&address=${encodeURIComponent(attraction.address)}`}
              ...
              {attraction.openTime && (
                <div>
                  ...
                  <div className="text-gray-900">{attraction.openTime}</div>
                </div>
              )}
              
              {attraction.ticketInfo && (
                <div>
                  <div className="text-sm text-gray-500 mb-1">门票信息</div>
                  <div className="text-gray-900">{attraction.ticketInfo}</div>
                </div>
              )}
            </div>
          </div>
